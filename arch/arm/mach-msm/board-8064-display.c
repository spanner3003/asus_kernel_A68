...
#define MSM_FB_PRIM_BUF_SIZE roundup( ALIGN(1920,32) * 1200 * 4 * 3, 0x10000) + roundup( ALIGN(720,32) * 1280 * 4 * 2, 0x10000) //Mickey, add for DDS transformation
#else
/* prim = 1366 x 768 x 3(bpp) x 2(pages) */
//#define MSM_FB_PRIM_BUF_SIZE roundup(1920 * 1080 * 4 * 2, 0x10000)
#define MSM_FB_PRIM_BUF_SIZE roundup( ALIGN(1920,32) * 1200 * 4 * 2, 0x10000) + roundup( ALIGN(720,32) * 1280 * 4 * 2, 0x10000) //Mickey, add for DDS transformation
#endif
//Mickey---

#ifdef CONFIG_FB_MSM_HDMI_MSM_PANEL
#define MSM_FB_EXT_BUF_SIZE \
		(roundup(( ALIGN(1920,32) * 1200 * 2), 4096) * 1) /* 2 bpp x 1 page */ //Mickey modified for 1920x1200 support
...
{
    pr_info("%s: on=%d\n", __func__, on);
    return a68_mipi_dsi_sharp_panel_power(on);
}
// --- ASUS_BSP : A68 miniporting
...
static void asus_draw_screen(struct work_struct *work);
static struct work_struct asus_draw_screen_work;
static struct workqueue_struct *drawscreen_workqueue;
static int g_image_color = 0x0;

void asus_draw_last_screen(u32 img_color)
{
    g_image_color = img_color;
    printk("[Display] queue a work to draw screen\n");
    queue_work(drawscreen_workqueue, &asus_draw_screen_work);
...
static void asus_draw_screen(struct work_struct *work)
{
    ...
    img_col[0] = (g_image_color & 0xFF000000) >> 24;
    img_col[1] = (g_image_color & 0x00FF0000) >> 16;
    img_col[2] = (g_image_color & 0x0000FF00) >> 8;
    img_col[3] = (g_image_color & 0x000000FF) ;
    ...
    //update_screen_flag = false;  //do not update screen forever
...
{
	//++ BSP: Louis
    drawscreen_workqueue = create_singlethread_workqueue("drawscreen_wq");
    INIT_WORK(&asus_draw_screen_work, asus_draw_screen); 
	//-- BSP: Louis
