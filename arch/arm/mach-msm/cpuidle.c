	...
	struct cpuidle_device *dev, struct cpuidle_state *state)
{
	...
	enum msm_pm_sleep_mode sleep_mode =
				(enum msm_pm_sleep_mode)state->driver_data;
...
#ifdef CONFIG_MSM_SLEEP_STATS
	atomic_notifier_call_chain(head, MSM_CPUIDLE_STATE_ENTER,
							(void *)sleep_mode);
#endif
	...
	cpu_pm_enter();
#endif
	ret = msm_pm_idle_enter(sleep_mode);
...
#ifdef CONFIG_MSM_SLEEP_STATS
	atomic_notifier_call_chain(head, MSM_CPUIDLE_STATE_EXIT,
							(void *)sleep_mode);
#endif
