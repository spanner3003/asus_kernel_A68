...
static int p03_set_backlight(int);
...
#endif
// ++ cabc on/off
        if (index <= 28 && CABA_On == true)
        {
            sharp_set_cabc(0);
            CABA_On = false;
        }
        else if (index > 28 && CABA_On == false)
        {
            sharp_set_cabc(3);
            CABA_On = true;
        }
...
static int p03_set_backlight(int value)
{
    int ret, index = 0;
                    ...
                    enum led_brightness value)
{
    ...
    if (backlight_mode_state == A68) {
        a68_set_backlight(backlight_value);
    }

    else if (backlight_mode_state == P03) {
...
static int change_backlight_mode(struct notifier_block *this, unsigned long event, void *ptr)
{
        switch (event) {
        case P01_ADD:
                backlight_mode_state = P03;
                p03_set_backlight(backlight_value);
                return NOTIFY_DONE;

        case P01_REMOVE:
                backlight_mode_state = A68;
                a68_set_backlight(backlight_value);
                return NOTIFY_DONE;
