...
static bool is_storage_suspend = 0;
//ASUS_BSP--- BennyCheng "add none mode switch for use storage case"
...
//ASUS_BSP+++ "[USB][NA][Other] support OTG Host and carkit"
...
//ASUS_BSP+++ BennyCheng "add none mode switch for use storage case"
void msm_otg_storage_suspend(void)
{
	struct msm_otg *motg = the_msm_otg;
	struct otg_transceiver *otg = &motg->otg;
...
static void msm_otg_late_resume_work(struct work_struct *w)
{
	...
	if (pad_hub_pm_enable) {
		...
		if (wait >= 10) {
			dev_err(otg->dev, "not b_idle state, skip host auto switch (%d)\n", otg->state);
			return;
		}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	if (g_otg_mode == ASUS_OTG_CONNECT) {
		...
		if (g_carkit_state) {
			...
			printk("[usb_otg] switch to client mode (OTG)\r\n");
			g_host_mode = 0;
			set_bit(ID, &motg->inputs);
			cancel_delayed_work_sync(&early_suspend_delay_work);
			cancel_work_sync(&late_resume_work);
		...
		if ((!test_bit(ID, &motg->inputs) ||
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					if (g_otg_mode == ASUS_OTG_CONNECT) {
						//switch to host
						printk("[usb_otg] switch to host mode (OTG)\r\n");
						g_otg_mode = ASUS_OTG_HOST;
						clear_bit(ID, &motg->inputs);
						g_host_mode = 1;
						work = 1;
...
static void msm_otg_set_id_state(int online)
{
	...
	if (g_A68_hwID < A68_PR) {
		dev_err(motg->otg.dev, "OTG not support for the HW!(%d)\n", g_A68_hwID);
		return;
	}

	if (USB_AUTO != motg->otg_mode) {
		dev_err(motg->otg.dev, "OTG not support for non AUTO mode!(%d)\n", motg->otg_mode);
		return;
	}
...
//ASUS_BSP+++ "[USB][NA][Spec] Add ASUS charger mode support"
		...
		registerChargerI2CReadyNotificaition(&msm_otg_charger_ready);//Eason : support OTG mode
#else
