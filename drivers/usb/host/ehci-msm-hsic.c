...
//ASUS_BSP+++ BennyCheng "check hsic idle time and free wakelock if over idle time allowed"
...
//ASUS_BSP+++ BennyCheng "workaround for power on crash due to ulpi_read jiffies issue"
...
static int msm_hsic_suspend(struct msm_hsic_hcd *mehci)
{
	...
	dev_info(mehci->dev, "HSIC-USB in low power mode (%d)\n", atomic_read(&mehci->dev->power.usage_count));
...
skip_phy_resume:
	...
	dev_info(mehci->dev, "HSIC-USB exited from low power mode (%d)\n", atomic_read(&mehci->dev->power.usage_count));
...
static irqreturn_t msm_hsic_wakeup_irq(int irq, void *data)
{
	...
	//ASUS_BSP+++ BennyCheng "enable hsic wakeup int debug msg"
...
static int __devinit ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	//ASUS_BSP+++ BennyCheng "separate ITC settings of HSIC and USB1"
	pdata = mehci->dev->platform_data;
	if (pdata)
