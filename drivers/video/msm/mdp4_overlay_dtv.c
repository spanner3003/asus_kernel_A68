...
void mdp4_hdmi_init(void);//Mickey+++
...
static int mdp4_dtv_start(struct msm_fb_data_type *mfd)
{
    ...
    //Mickey+++, reinit to prevent no overlay after suspend
    else
        mdp4_hdmi_init();
    //Mickey---
	...
	g_QCOM_TMDS_ready = 1;
    printk("qseed=0x%x,%s\n",inpdw(0xe1928100),__func__);//Mickey+++, add for grid on overlay debugging
	return 0;
}
...
//Mickey+++
static void dtv_overlay(struct msm_fb_data_type *mfd)
{
...
static void dtv_overlay_restore(struct msm_fb_data_type *mfd)
{
    if (dtv_pipe == NULL)
    {
        dtv_overlay(mfd);
...
extern bool g_p03Exist;

#ifdef CONFIG_MSM_BUS_SCALING
#define OVERLAY_BUS_SCALE_TABLE_BASE	6
int mdp_bus_scale_update_request(uint32_t);
#endif
//Mickey---
