...
struct msm_fb_data_type *g_mfd;     //+++ ASUS_BSP : miniporting
...
static char bl_value[] = {0x51, 0x0B};      //write brightness
...
//+++ read config register for ESD test
...
static bool write_lcd_reg_value(char addr, uint32 value)
{
    ...
    sprintf(writeaddr, "0x%x:", addr);
    fp->f_op->write(fp, writeaddr, strlen(writeaddr), &pos_lsts);

    sprintf(writeval, "0x%x\n", value);
    fp->f_op->write(fp, writeval, strlen(writeval), &pos_lsts);
...
static uint32 dump_mipi_novatek_register(struct msm_fb_data_type *mfd)
{
    ...
    {
        cmd = &dump_novatek_register_cmd[i];
...
int sharp_set_brightness(int value)
{
    ...
    if (mdp4_overlay_dsi_state_get() == ST_DSI_SUSPEND) {
        mutex_unlock(&g_mfd->dma->ov_mutex);
        printk("[Display] %s: unable to send DCS command due to ST_DSI_SUSPEND\n", __func__);
        return -1;
    }
    /* mdp4_dsi_cmd_busy_wait: will turn on dsi clock also */
    mdp4_dsi_cmd_dma_busy_wait(g_mfd);
    mdp4_dsi_blt_dmap_busy_wait(g_mfd);
    mipi_dsi_mdp_busy_wait(g_mfd);
    ...
    if (value == 0 && bl_dimming == true) {
        ...
        mipi_dsi_cmds_tx(g_mfd, &novatek_tx_buf, novatek_brightness_set_suspend,
            ARRAY_SIZE(novatek_brightness_set_suspend));
        ...
        mipi_dsi_cmds_tx(g_mfd, &novatek_tx_buf, novatek_brightness_set,
            ARRAY_SIZE(novatek_brightness_set));
...
EXPORT_SYMBOL(sharp_set_brightness);
...
void sharp_panel_on(void)
{
    ...
    //panel_display_on[0].payload[1] = g_bl_value;
    //panel_display_on[1].payload[1] = 0x2C;

    mipi_dsi_cmds_tx(g_mfd, &novatek_tx_buf, panel_display_on,
                ARRAY_SIZE(panel_display_on));
...
int g_skipOVframes = 0;//Mickey+++, add a workaround for MDP composition hang
static int mipi_novatek_lcd_on(struct platform_device *pdev)
{
    ...
    g_skipOVframes = 1;//Mickey+++, add a workaround for MDP composition hang
    ...
    if(bFirst) {
#ifdef CONFIG_FB_MSM_MIPI_NOVATEK_VIDEO_MODE
            mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, a68_init_cmds,
                ARRAY_SIZE(a68_init_cmds));
#endif
    ...
    mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_display_on,
        ARRAY_SIZE(novatek_display_on));

    g_displayOn = true;

    printk("[Display] novatek LCD on\n");
    return 0;
// ASUS_BSP: for miniporting ---
...
{
	struct msm_fb_data_type *mfd;
    struct mipi_panel_info *mipi;	// +++ ASUS_BSP : miniporting
	mfd = platform_get_drvdata(pdev);
...
void sharp_set_cabc(int mode)
{
    ...
    if (mode == 4) {
        ...
        mipi_dsi_cmds_tx(g_mfd, &novatek_tx_buf, cabc_function_off,
                    ARRAY_SIZE(cabc_function_off));
        mutex_unlock(&g_mfd->dma->ov_mutex);
        return;
    }
    ...
    printk("[Display] write cabc mode = 0x%x\n", IE_CABC[1]);

    mipi_dsi_cmds_tx(g_mfd, &novatek_tx_buf, set_cabc_cmd,
                ARRAY_SIZE(set_cabc_cmd));
