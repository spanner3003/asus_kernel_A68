...
spinlock_t dsi_clk_lock;//QCT patch+++
...
void mipi_dsi_init(void)
{
    ...
    spin_lock_init(&dsi_clk_lock);//QCT patch+++
		...
		struct dsi_buf *tp, struct dsi_cmd_desc *cmds, int cnt)
{
	...
	*/
    ...
    mipi_dsi_mdp_busy_wait(mfd);//Mickey+++, add to avoid dsi race condition
	spin_lock_irqsave(&dsi_mdp_lock, flag);
	mipi_dsi_enable_irq();
	dsi_mdp_busy = TRUE;
	spin_unlock_irqrestore(&dsi_mdp_lock, flag);
...
int mipi_dsi_cmd_dma_tx(struct dsi_buf *tp)
{
    ...
    //Mickey+++, don't block system event command no response
    //wait_for_completion(&dsi_dma_comp);
    if (!wait_for_completion_timeout(&dsi_dma_comp,HZ))//change to 1 seconds
        printk("****************DCS TX command timeout****************\n");
    //Mickey---
...
irqreturn_t mipi_dsi_isr(int irq, void *ptr)
{
	...
	if (isr & DSI_INTR_CMD_MDP_DONE) {
        ...
        complete(&dsi_mdp_comp);//Mickey change to here to prevent race condiction
