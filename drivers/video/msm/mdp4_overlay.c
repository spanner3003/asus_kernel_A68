...
//Mickey+++, for switch to pad mode used
void asus_mdp4_mixer0_pipe_cleanup(struct fb_info *info)
{
    struct mdp4_overlay_pipe *pipe;
    int j;
	...
	struct msm_fb_data_type *mfd, uint32 perf_level)
{
    ...
    //Mickey+++
    if (req->src_rect.w >= 1900 && req->src_rect.h >= 1000 && req->dst_rect.w <= 764 && req->dst_rect.h <= 430)
        use_blt = 1;
    //Mickey---
...
//Mickey+++
void dumpOVPipeInfo(struct mdp4_overlay_pipe *pipe)
{
    printk("T=%d,M=%d\n",pipe->pipe_type,pipe->mixer_num);
    printk("Sx%dy%dw%dh%d,Dx%dy%dw%dh%d\n",pipe->src_x,pipe->src_y,pipe->src_w,pipe->src_h,pipe->dst_x,pipe->dst_y,pipe->dst_w,pipe->dst_h);
}
//Mickey---
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
    ...
    if (g_p03Exist)
    {
        if ((req->dst_rect.w == 919 && req->dst_rect.h == 751) && (req->src_rect.w <= 352 && req->src_rect.h <= 288))
                return -999;
        if ((req->dst_rect.w == 654 && req->dst_rect.h == 800) && (req->src_rect.w == 288 && req->src_rect.h == 352))
                return -999;
    }
    //Mickey---
    ...
    //Mickey+++,we don't use blt in pad mode
    perf_level = mdp4_overlay_get_perf_level(req, mfd);
    if (g_p03Exist && info->node == 1)
        if (mdp4_overlay_blt_enable(req, mfd, perf_level))
            return -999;
    //Mickey---
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	...
	if (pipe == NULL) {
		mutex_unlock(&mfd->dma->ov_mutex);
        return 0;//-ENODEV;//Mickey+++, we treat NULL pipe as unset successfully
	}
...
int mdp4_overlay_play(struct fb_info *info, struct msmfb_overlay_data *req)
{
	...
	int ret = 0;
    static unsigned int qseed = 0;//Mickey+++, add for grid on overlay debugging
    ...
    //Mickey+++, add for grid on overlay debugging
    if (pipe->pipe_type == OVERLAY_TYPE_VIDEO)
    {
        if (qseed != inpdw(0xe1928100))
        {
            qseed = inpdw(0xe1928100);
            printk("qseed=0x%x,%s\n",qseed,__func__);
        }
    }
    //Mickey---
