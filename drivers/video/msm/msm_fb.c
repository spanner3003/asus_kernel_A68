...
void asus_fb0_screen_suspend(bool suspend)
{
    ...
    if (suspend)
    {
        ...
        if (g_fb0_on)
        {
            asus_mdp4_mixer0_pipe_cleanup(mfd_list[0]->fbi);//Mickey+++, clean up all pipe before we going to sleep
...
static void msmfb_early_suspend(struct early_suspend *h)
{
	...
	*/
    ...
    if (mfd->fbi->node == 0)
    {
        ...
        {
            asus_mdp4_mixer0_pipe_cleanup(mfd->fbi);//Mickey+++, clean up all pipe before we going to sleep
...
//Mickey+++, add for DDS architecture
    ...
    {
#ifdef CONFIG_FB_MSM_TRIPLE_BUFFER
        fix->smem_len = roundup(ALIGN(panel_info->asus_dds_xres,32) * panel_info->asus_dds_yres * 4 * 3, 0x10000) + roundup(ALIGN(panel_info->xres,32) * panel_info->yres * 4 * 2, 0x10000);
#else
        fix->smem_len = roundup(ALIGN(panel_info->asus_dds_xres,32) * panel_info->asus_dds_yres * 4 * 2, 0x10000) + roundup(ALIGN(panel_info->xres,32) * panel_info->yres * 4 * 2, 0x10000);
#endif
...
static int msmfb_overlay_set(struct fb_info *info, void __user *p)
{
	...
	if (ret) {
        if (ret != -999)//Mickey+++, we skip the error case
