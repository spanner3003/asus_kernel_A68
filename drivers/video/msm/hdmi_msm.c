...
{
    g_p03Exist = gpio_get_value(ASUS_P03_STATUS_GPIO);
    return g_p03Exist;
}
...
static void hdmi_msm_setup_video_mode_lut(void)
{
    ...
    HDMI_SETUP_LUT(1920x1200p60_16_10);//Mickey
}
...
static void hdmi_msm_hpd_state_work(struct work_struct *work)
{
	...
	if ((external_common_state->hpd_state != hpd_state) || (hdmi_msm_state->
        ...
        wake_lock_timeout(&hdmi_msm_state->hdp_wake_lock,5*HZ);//Mickey+++, lock a wake lock for 5 sec when hpd detect
        printk("[Mickey]hwl5!\n");
		return;
	}
	mutex_unlock(&external_common_state_hpd_mutex);
...
//ASUS_BSP: larry lai tune hdmi swing, default = 0xf2
//	HDMI_OUTP(0x0404, 0x000000F5);
	
//	printk("hdmi_msm_turn_on : HDMI_PHY_REG_1 = 0x%x\n", HDMI_INP(0x0404) );
...
#define HDMI_HPD_GPIO		72//Mickey+++
static int hdmi_msm_power_ctrl(boolean enable)
{
	int rc = 0;
    //Mickey+++, trigger HPD state work if HPD pin is changed
    static int preHPD = 0;
    bool recheckHPD = false;
    //Mickey---
	if (enable) {
		/*
		 ...
		 */
        //Mickey+++, trigger HPD state work if HPD pin is changed
        if (preHPD != gpio_get_value(HDMI_HPD_GPIO))
            recheckHPD = true;
        //Mickey---
		if (hdmi_prim_display ||
			external_common_state->hpd_feature_on) {
			DEV_DBG("%s: Turning HPD ciruitry on\n", __func__);
            rc = hdmi_msm_hpd_on(recheckHPD);//Mickey+++
            printk("[Mickey]HPDon(pre:%d,recheckHPD:%d)\n", preHPD,recheckHPD);
		}
	} else {
		DEV_DBG("%s: Turning HPD ciruitry off\n", __func__);
		hdmi_msm_hpd_off();
        preHPD = gpio_get_value(HDMI_HPD_GPIO);//Mickey+++
		printk("[Mickey]HPDoff(pre:%d,recheckHPD:%d)\n", preHPD,recheckHPD);	
	}
...
static int __devinit hdmi_msm_probe(struct platform_device *pdev)
{
    ...
    wake_lock_init(&hdmi_msm_state->hdp_wake_lock, WAKE_LOCK_SUSPEND, "hdp_wake");//Mickey+++
...
static int __init hdmi_msm_init(void)
{
	...
	external_common_state->video_resolution = HDMI_VFRMT_1920x1200p60_16_10;//Mickey change for 1920x1200 support
#ifdef CONFIG_FB_MSM_HDMI_3D
