...
//Qualcomm++
	...
	dev->wr_comp = &done;
	//msm_send_msg_buf(ctrl, buf, 9);       //Qualcomm++
    ret = msm_send_msg_buf(ctrl, buf, 9);   //Qualcomm++
	timeout = wait_for_completion_timeout(&done, HZ);
//Qualcomm++
    if (!timeout)
        dev->err = -ETIMEDOUT;
...
static void slim_sat_rxprocess(struct work_struct *work)
{
	...
	while ((msm_sat_dequeue(sat, buf)) != -ENODATA) {
		...
		if (mt == SLIM_MSG_MT_CORE &&
			...
			//slim_assign_laddr(&dev->ctrl, e_addr, 6, &laddr);     //Qualcomm++
			//sat->satcl.laddr = laddr;                             //Qualcomm++
			/*
