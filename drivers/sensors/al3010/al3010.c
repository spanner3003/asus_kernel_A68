...
/*For resume I2C issue*/
static struct delayed_work Al3010light_resume_work;
static struct workqueue_struct *Al3010light_resume_wq = NULL;
bool g_al3010_suspend_switch_on = false;
	

struct i2c_client *al3010_client = NULL;

static int TOTALMAPS = 16;
static int g_al3010_light_map[16] = 
    {0,50,100,150,200,250,300,350,450,550,700,850,1050,1250,1500,2200} ;   
 ...
 */
static int g_polling_count = 0;
static int polling_lux = 0;
...
void al3010_poll_work(struct work_struct *work)
{
	...
	if ( lux == 0 )
		...
		lux = g_al3010_light_shift_calibration + (lux * g_al3010_light_calibration);
...
int set_als_power_state_of_P01(int state)
{
	...
	for(indx = 0; indx<5; indx++) {
		ret = al3010_init_client(al3010_client);
		if(!ret)	{
			printk(DBGMSK_PRX_G2"[al3010][als] init_client al3010 success\n");
			break;
		}else	{
...
static int al3010_get_adc_value(struct i2c_client *client)
{
	...
	/*Get Lux*/
	int i,k_adc,lux =0;
	k_adc = adc * g_al3010_light_calibration;
	for( i=1 ; i < TOTALMAPS ; i++) {
		if ( k_adc > 40 && k_adc < g_al3010_light_map[ 2 ])	{
			lux = g_al3010_light_map[ 1 ];
			break;
		}else	{
			if( k_adc < g_al3010_light_map[i] ) {
				lux = g_al3010_light_map[ i -1 ];
				break;
			}
			else if( k_adc > g_al3010_light_map[TOTALMAPS - 1] )	{
				lux = g_al3010_light_map[ TOTALMAPS -1 ];
				break;
			}
		}
	}
	if( lux > g_al3010_light_map[TOTALMAPS - 1] )
		lux = g_al3010_light_map[TOTALMAPS - 1];
	printk(DBGMSK_PRX_G2"[als_P02] lux now= %d, raw adc= %d, cal_adc= %d, lux = %d\n",lux, adc, k_adc, g_al3010_light);
...
static int al3010_put_property(struct i2c_client *client)
{
      ...
      //  __al3010_write_reg(al3010_client, AL3010_ALS_COMMAND, AL3010_INT_IF, AL3010_INT_SHIFT, ALS_IF);
        ...
        ret = i2c_smbus_write_byte_data(client, AL3010_INT_COMMAND,  ALS_IF | (ALS_Gain << 4 ));
        if (ret < 0)  {
            printk(DBGMSK_PRX_G2"[als_P02] addr=0x%x, val=0x%x, ret=%d\n",AL3010_INT_COMMAND, ALS_IF | (ALS_Gain << 4 ), ret);
...
static void al3010_late_resume_delayed_work(struct work_struct *work)
{
	polling_lux = 0;
	g_polling_count++;
	if ( g_polling_count == 1 )	{
	printk(DBGMSK_PRX_G2"[als_P01] al3010_late_resume, resume ALS\n");
	set_als_power_state_of_P01(g_al3010_suspend_switch_on);
		queue_delayed_work(Al3010light_resume_wq, &Al3010light_resume_work, (HZ * 3)) ; 
	}else if ( g_polling_count > 1 && g_polling_count < 3 )	{
		polling_lux = al3010_get_adc_value(al3010_client);
		if( polling_lux < 0 )
			printk(DBGMSK_PRX_G6"[als_P01] al3010_poll_work: failed err=%d\n", polling_lux );
		else	{
			als_lux_report_event( polling_lux );
			printk(DBGMSK_PRX_G6"[als_P01] al3010_poll_work: lux=%d\n", polling_lux );
		}
		queue_delayed_work(Al3010light_resume_wq, &Al3010light_resume_work, (HZ * 3)) ; //100ms.	
	}
	else	
		g_polling_count = 0;
}
...
static void al3010_late_resume(struct early_suspend *handler)
{
	...
	if(1 == g_al3010_switch_earlysuspend) {
		...
		if( g_bIsP01Attached && g_al3010_suspend_switch_on ) {
			printk(DBGMSK_PRX_G2"[als_P01] al3010_late_resume, resume ALS +++\n");
			queue_delayed_work(Al3010light_resume_wq, &Al3010light_resume_work, HZ * 300/1000);
...
static int __init al3010_init(void)
{
	...
	/*For resume I2C issue*/
	Al3010light_resume_wq = create_workqueue("al3010light_resume_wq");
