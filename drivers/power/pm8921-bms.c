						...
						int *delta_time_us)
{
	...
	*unusable_charge_uah = calculate_unusable_charge_uah(chip, *rbatt,
	...
	printk("[BAT][BMS]force ER UUC=0+++\n");
	*unusable_charge_uah = 0;
	printk("[BAT][BMS]force ER UUC=0---:UUC = %uuAh\n",*unusable_charge_uah);
}
...
//ASUS BSP Eason_Chang: take off adjust_for_shutdown_soc prevent limit percent+++
#if 0
					...
					int batt_temp, int chargecycles)
{
	...
	int cc_uah;
	int rbatt;
	//int shutdown_adjusted_soc;//ASUS BSP Eason_Chang: take off adjust_for_shutdown_soc prevent limit percent
	int iavg_ua;
	int delta_time_us;
...
//ASUS BSP Eason: ER need force last_soc = soc +++
#if 0
	if (last_soc == -EINVAL || soc <= last_soc) {
		...
		if (the_chip->start_percent != -EINVAL)
			...
			printk("soc = %d reporting last_soc = %d\n", soc,
								last_soc);
	}
#endif
		last_soc = soc;
